...
class LoRaWANModule_RUI3(RUI3):
    ...
    def __init__(self, id: Literal[0, 1, 2] = 1, tx=-1, rx=-1, rst=-1, debug=False):
        super().__init__(id, tx, rx, rst, debug)
        if self.get_serial_number() is False:
            self.close()
            raise ValueError(
                "The LoRaWAN-X Module is not responding. Please check the connection."
            )
